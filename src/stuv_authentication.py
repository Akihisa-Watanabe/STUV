...
from matcher import BasicMatcher, STUVMatcher
...
def authentication_scenario(args):
    data, genuine_user, m, t_now = args
    ...
    # print(f"Feature Users: {feature_users}")
    # print(f"Classifier Users: {classifier_users}")
    # print(f"Imposter Users: {imposter_users}")
    ...
    matcher = STUVMatcher(model_type="neural")
    X_train_genuine = T.drop(["time"], axis=1).values
    genuine_time_labels = T["time"].values.reshape(-1, 1)
    matcher.fit(
        X_train_genuine,
        genuine_time_labels,
        X_train_classifier,
        classifier_user_labels.squeeze(),
    )
    ...
    for t in range(1, t_now + 1):
        ...
        if t < t_now:
            ...
            replay_prob_df_t = pd.DataFrame(
                matcher.predict_proba(X_Replay_t, t_now)[:, 1], columns=["prob1"]
            )
            ...
            replay_prob_df_t = pd.DataFrame(
                matcher.predict_proba(X_test_genuine_tnow, t_now)[:, 1],
                columns=["prob1"],
            )
            # print('replay_prob_df_t', replay_prob_df_t)
            replay_prob_df_t["true_user"] = genuine_user
            replay_prob_df_t["test_user"] = genuine_user
            replay_prob_df_t["threshold"] = 0.5
            replay_prob_df_t["time"] = t
            ...
            replay_prob_df_t_imposter = pd.DataFrame(
                matcher.predict_proba(X_test_imposter_tnow, t_now)[:, 1],
                columns=["prob1"],
            )
...
if __name__ == "__main__":
    ...
    for genuine_user in tqdm(all_users):
        pairs.append((data.copy(), genuine_user, m, t_now))

    with Pool(cpu_count()) as p:
        imap = p.imap(func=authentication_scenario, iterable=pairs)
        result = list(tqdm(imap, total=len(pairs), leave=False, desc="Processing"))
        p.close()

    feature_df = pd.concat([r[0] for r in result])
    mather_df = pd.concat([r[1] for r in result])
    replay_feature_df = pd.concat([r[2] for r in result])
    replay_prob_df = pd.concat([r[3] for r in result])

    feature_df.to_csv(
        "../result/STUV/Normal/nn_replay_aware_feature_df.csv", index=False
    )
    mather_df.to_csv(
        "../result/STUV/Normal/nn_replay_aware_mather_df.csv", index=False
    )
    replay_prob_df.to_csv(
        "../result/STUV/Normal/nn_replay_aware_replay_prob_df.csv", index=False
    )
    replay_feature_df.to_csv(
        "../result/STUV/Normal/nn_replay_aware_replay_feature_df.csv", index=False
    )
